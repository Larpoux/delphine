...
console.log('[boot] typeof acquireVsCodeApi =', typeof (globalThis as any).acquireVsCodeApi);
...
function findAcquireVsCodeApiFn(): (() => VsCodeApi) | undefined {
        ...
        const frames: any[] = [globalThis, window, window.parent, window.top];

        for (const f of frames) {
                try {
                        const fn = f?.acquireVsCodeApi;
                        if (typeof fn === 'function') return fn as () => VsCodeApi;
                } catch {
...
function getVsCodeApi(): VsCodeApi | undefined {
        try {
                // In VS Code webview this exists
...
function grapesJSEditor(grapesJs: GrapesJsApi) {
...
function waitForGrapesJs(timeoutMs = 5000): Promise<GrapesJsApi> {
        const start = Date.now();

        return new Promise((resolve, reject) => {
                const tick = () => {
                        const api = window.grapesjs;
                        if (api && typeof api.init === 'function') return resolve(api);
